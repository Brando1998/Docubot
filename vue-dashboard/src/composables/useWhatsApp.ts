...
export function useWhatsApp() {
  ...
  const fetchWhatsAppStatus = async () => {
    try {
      ...
      error.value =
        err.response?.data?.error || err.message || "Error obteniendo estado";
  ...
  const generateQR = async () => {
    try {
      ...
      if (
        whatsappData.value?.status !== "waiting_for_scan" &&
        whatsappData.value?.status !== "initializing"
      ) {
      ...
      error.value =
        err.response?.data?.error || err.message || "Error generando QR";
  ...
  const refreshQR = async () => {
    try {
      ...
      error.value =
        err.response?.data?.error || err.message || "Error actualizando QR";
  ...
  const disconnectWhatsApp = async () => {
    try {
      ...
      error.value =
        err.response?.data?.error || err.message || "Error desconectando";
  ...
  const restartSession = async () => {
    try {
      ...
      error.value =
        err.response?.data?.error || err.message || "Error reiniciando sesión";
  ...
  const getDetailedStatus = async () => {
    try {
      ...
      error.value =
        err.response?.data?.error ||
        err.message ||
        "Error obteniendo estado detallado";
  ...
  const sendMessage = async (number: string, message: string) => {
    try {
      ...
      error.value =
        err.response?.data?.error || err.message || "Error enviando mensaje";
  ...
  const getChats = async () => {
    try {
      ...
      error.value =
        err.response?.data?.error || err.message || "Error obteniendo chats";
  ...
  const getChatMessages = async (chatId: string) => {
    try {
      ...
      const response = await api.get(
        `/api/v1/whatsapp/chats/${chatId}/messages`
      );
      return response.data;
    } catch (err: any) {
      error.value =
        err.response?.data?.error || err.message || "Error obteniendo mensajes";
