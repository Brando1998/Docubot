<template>
  <div class="max-w-4xl mx-auto p-6">
      ...
      <div class="p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <span :class="statusBadgeClasses">
            <span
              :class="['w-2 h-2 rounded-full mr-2', statusDotClasses]"
            ></span>
            {{ statusText }}
          </span>
        </div>
        ...
        >
          <div class="flex items-center">
            ...
            <div>
              ...
              <p v-if="whatsappData.user_name" class="text-sm text-green-600">
                Usuario: {{ whatsappData.user_name }}
              </p>
              <p
                v-if="whatsappData.phone_number"
                class="text-sm text-green-600"
              >
                Número: {{ whatsappData.phone_number }}
              </p>
              <p
                v-if="whatsappData.last_connected"
                class="text-sm text-green-600"
              >
                Última conexión: {{ formatDate(whatsappData.last_connected) }}
              </p>
            </div>
          </div>
        </div>
    ...
    <!-- QR Code Display -->
    <div
      v-if="whatsappData?.qr_image && !whatsappData?.connected"
    ...
    >
      <div class="p-6 text-center">
        ...
        <p class="text-sm text-gray-600">
          Abre WhatsApp en tu teléfono, ve a Dispositivos vinculados y escanea
          este código
        </p>
      </div>
    </div>
    ...
    <!-- Estado Sin Conexión -->
    <div
      v-if="!whatsappData?.connected && !whatsappData?.qr_image"
      class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6"
    >
      <div class="p-6 text-center">
        <div
        ...
        <p class="text-gray-600 text-sm mb-4">
          Genera un código QR para conectar WhatsApp
        </p>
      </div>
    </div>
      ...
      <div class="p-6">
        <div class="text-center">
          ...
          <p class="mt-1 text-sm text-gray-500">
            Administra tus conversaciones de WhatsApp
          </p>
          <router-link
            to="/dashboard/chats"
...
const statusText = computed(() => {
  ...
  if (whatsappData.value.status === "waiting_for_scan")
...
const statusBadgeClasses = computed(() => {
  ...
  if (whatsappData.value.connected)
    return `${base} bg-green-100 text-green-800`;
  if (whatsappData.value.status === "waiting_for_scan")
    return `${base} bg-blue-100 text-blue-800`;
  if (whatsappData.value.status === "initializing")
    return `${base} bg-yellow-100 text-yellow-800`;
  if (whatsappData.value.status === "restarting")
    return `${base} bg-orange-100 text-orange-800`;
  if (whatsappData.value.status === "cleared")
...
const messageClasses = computed(() => {
  const base = "border";
  if (uiMessageType.value === "success")
...
const handleClearSession = async () => {
  try {
    ...
    if (
      confirm(
        "¿Estás seguro de que quieres limpiar las credenciales? Esto eliminará la sesión actual."
      )
    ) {
      await clearSession();
