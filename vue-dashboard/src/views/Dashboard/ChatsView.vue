  ...
  <div class="h-screen flex bg-gray-50">
        ...
        <div v-if="filteredChats.length > 0" class="divide-y divide-gray-100">
          ...
          >
            <div class="flex items-center justify-between mb-2">
              <h4 class="font-medium text-gray-900 text-sm truncate flex-1">{{ chat.name }}</h4>
              <div class="flex items-center space-x-1">
                <span v-if="chat.unreadCount > 0" class="bg-red-500 text-white text-xs px-2 py-1 rounded-full">
                  {{ chat.unreadCount }}
                </span>
                <span v-if="chat.isGroup" class="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">
                  G
                </span>
              </div>
            </div>
            ...
            <div class="flex items-center justify-between">
              ...
              <span :class="[
                ...
                chat.botMode ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
              ]">
    ...
    <div class="flex-1 flex flex-col bg-white">
      ...
      <div class="flex-1 overflow-y-auto p-4" ref="messagesContainer">
        <div v-if="!selectedChat" class="flex items-center justify-center h-full text-center">
          <div>
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
            </svg>
            <h3 class="mt-2 text-sm font-medium text-gray-900">Selecciona un chat</h3>
            <p class="mt-1 text-sm text-gray-500">Elige un chat de la lista para ver la conversación</p>
          </div>
        </div>
...
const toggleChatMode = async (chat: any) => {
  ...
  try {
    ...
    showMessage(`Modo ${newMode ? 'bot' : 'humano'} activado para ${chat.name}`, 'success')
  } catch (err: any) {
